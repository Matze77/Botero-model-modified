...
params4=[[[0,0.90,"RP",1,1.2],[2,0.90,"IP",1,1.1]]] #rp-ip
params5 =[[[0,0.10,"CBH",1,2.1],[1,0.10,"DBH",1,2.5]]] #cbh-dbh
params6=[[[2,0.10,"DBH",1,3.8],[3.5,0.10,"AT",1,1.1]]] #dbh-at



for s,pair in enumerate(params6):
    R=[]
    P=[]
    strat=[]
    desc=[]
...
#    
        ...
        if strat[i] in ["RP","IP"]:
            modes=[["normal",0,0],["rand",1,0],["rand_disc",1,1]]
        ...
        for k in modes:            
            ...
            for j in [["mut_0.001",0.001,0.0],["sc_0.05",0.0,0.05],["mut_0.001_sc_0.05",0.001,0.05],["hgt_r",0,0],["hgt_c",1,0]]:
                q=Q[i] 
                stop=False
                while not stop:  
                    ...
                    if j[0][0]!="h":
                        ...
                        --plot_every 50 --populations 20 --generations 1000 --stop_half 1 --survival_goal 0.5 --stop_below 0.5 800 --use_pop {6} --mutation 0.001 {7} 0.05 {8} --random_a_b {10} --discrete_s {11}".format(R[i],P[i],transition,path1[(i+1)%2],q,j[0],use_pop[(i+1)%2],j[1],j[2],k[0],k[1],k[2]) 
                    else:  
                        p=" --environment {0} {1} 1 0 0 --desc {2}_{4} --trans 1 --folder {9}/trans_{5} --path {3} --q {4}\
                        --plot_every 50 --populations 20 --generations 1000 --stop_half 1 --survival_goal 0.5 --stop_below 0.5 800  --hgt 1 --check {6} --use_pop {8}  --kt {7} --random_a_b {10} --discrete_s {11} ".format(R[i],P[i],transition,path1[(i+1)%2],q,j[0],j[1],j[2],use_pop[(i+1)%2],k[0],k[1],k[2]) 
