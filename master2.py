...
params1=[[[0,0.10,"CBH",1,2.1],[0,0.90,"RP",1,2.0]]]   #cbh-rp
params2=[[[2,0.10,"DBH",1,3.80],[2.0,0.90,"IP",1,2.5]]] #dbh-ip        
params3= [[[3.5,0.70,"AT",3,2.4],[2.5,0.70,"IP",2,2.4]]]  #ip-at
...
for s,pair in enumerate(params3):
    R=[]
    P=[]
    strat=[]
    desc=[]
    ...
    for i,param in enumerate(pair):
        ...
        path1.append(path+"genes/"+desc[i]+"/")
...
#    
        ...
        if strat[i] in ["RP","IP"] and strat[(i+1)%2] not in ["RP","IP"]:
            modes=[["evolve",1]]#["rand",1,0]]
        elif strat[(i+1)%2] in ["RP","IP"]:
            continue
            modes=[["normal",0],["random",1]]
        else: 
            continue
            modes=[["normal",0],["random",1]]
        for k in modes:            
             
            for j in [["mut_0.001",0.001,0.0],["sc_0.05",0.0,0.05],["mut_0.001_sc_0.05",0.001,0.05]]:#,["normal",0.,0.],["hgt_c",1,0],["hgt_r",0,0]]:
                q=Q[i] 
                stop=False
                while not stop:  
                    ...
                    if j[0][0]!="h":
                        ...
                        --plot_every 200 --populations 20 --generations 1000 --stop_half 1 --random_a_b {10} --survival_goal 0.5 --stop_below 0.5 800 --use_pop {6} --mutation 0.001 {7} 0.05 {8} ".format(R[i],P[i],transition,path1[(i+1)%2],q,j[0],use_pop[(i+1)%2],j[1],j[2],k[0],k[1]) 
                    else:  
                        p=" --environment {0} {1} 1 0 0 --desc {2}_{4} --trans 1 --folder {9}/trans_{5} --path {3} --q {4}\
                        --plot_every 200 --populations 20 --generations 1000 --stop_half 1 evolve_a_b {10} --survival_goal 0.5 --stop_below 0.5 800  --hgt 1 --check {6} --use_pop {8}  --kt {7}".format(R[i],P[i],transition,path1[(i+1)%2],q,j[0],j[1],j[2],use_pop[(i+1)%2],k[0],k[1]) 
...
#                --plot_every 50 --populations 10 --stop_below 0.5 800 --generations 1000 --random_a_b {6} --discrete_s {7} --survival_goal 0.5  --stop_half 1 --use_pop {8}".format(R[i],P[i],transition,path1[(i+1)%2],q,k[0],k[1],k[2],use_pop[(i+1)%2]) 
                   ...
                   # print(p)
                    os.system(c)
                    with open(path+"/output_variable/"+k[0]+"/trans_"+j[0]+"/"+transition+"_"+str(q)+"/__overview.txt") as f3:
                            b=f3.read()               
                    s=b[-27:-22]
                    s1=float(s.rsplit("/")[0])
                    s2=float(s.rsplit("/")[1])
